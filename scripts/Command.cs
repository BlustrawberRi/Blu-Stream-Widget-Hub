...
public class CommandData
{
    ...
    public bool GetDataFromAnswer(string answer)
    {
        string eventSource = "";
        try
        {
            var eventData = Json.ParseString(answer).AsGodotDictionary().GetValueOrDefault("event").AsGodotDictionary();
            eventSource = eventData.GetValueOrDefault("source").AsString();
        } 
		catch (Exception) {
            return false;
        }
